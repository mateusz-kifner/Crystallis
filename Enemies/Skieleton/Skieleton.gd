...
var player
onready var skieleton = $Armature001
onready var HAMDS = $Armature001/Skeleton/HAMDS
var angle = 0
var attack
var dead = false
...
func _physics_process(delta):
	...
	if not(dead) and not(is_damaged):
		if(attack) and $AnimationPlayer.current_animation != "attack":
			var player_origin =  skieleton.global_transform.origin - player.global_transform.origin
		
			var new_angle = (-Vector2(-1,0).angle_to(Vector2(player_origin.x,player_origin.z))) + (PI/2)
			print(new_angle) 
			skieleton.rotation = Vector3(0,new_angle,0)
			$AnimationPlayer.play("attack",-1,3)
			$Damage.start(0.8)
		if not(attack) and $AnimationPlayer.current_animation != "attack":
			if path_ind < path.size():
				if $AnimationPlayer.current_animation != "walk":
					$AnimationPlayer.play("walk",-1,6)
...
func _on_Damage_timeout():
	HAMDS.material_override = null
	if is_damaged :
		is_damaged = false
	if dead:
		queue_free()
	if attack and not (dead):
		player.damage(DAMAGE)

func damage(amount:int):
	HAMDS.material_override = load("res://Materials/Bones_Red.material")
	
	if health -amount > 0:
		health -= amount
		print("skieleton", health)
		is_damaged = true
		$Damage.start(0.5)
